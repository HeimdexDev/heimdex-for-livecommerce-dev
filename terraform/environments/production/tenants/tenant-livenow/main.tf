...
module "ec2" {
  ...
  user_data = templatefile("${path.module}/../../../../modules/ec2-instance/templates/user_data.sh.tpl", {
    ...
    ssm_prefix      = "/heimdex/prod/tenants/livenow"
    region          = "ap-northeast-2"
  ...
  extra_tags = {
    ClientDomain = "livenow.app.heimdex.co"
  }
}

# ============================================
# Elastic IP (import target: eipalloc-00a0a51f6461da26f)
# ============================================
resource "aws_eip" "this" {
  domain = "vpc"

  tags = {
    Name        = "heimdex-prod-livenow"
    Client      = "livenow"
    Environment = "production"
    ManagedBy   = "terraform"
  }
...
resource "aws_ssm_parameter" "ec2_host" {
  name  = "/heimdex/prod/tenants/livenow/EC2_HOST"
  type  = "String"
