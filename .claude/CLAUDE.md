...
Pipeline: VLM prompt (`heimdex-media-pipelines/vision/tagging.py`) uses 3 diverse rotating few-shot examples (shoes/kitchenware/bag) to prevent single-example poisoning → parser (`heimdex-media-contracts/tags/parser.py`) extracts and cleans via `_clean_ai_tags()` → caption worker sends to enrich API (gated by `ai_tags_enabled` in SQS message) → OpenSearch stores as `keyword` type with `nori` text sub-field → frontend displays in emerald pills (distinct from indigo structured tags)

**VLM Model**: Qwen2.5-VL-7B-Instruct with bitsandbytes 4-bit quantization (~6-7GB VRAM). Runs on Aircloud RTX 4070 TI Super (16GB VRAM, 58GB RAM). Engine key: `qwen2vl` (same class handles 2B and 7B via `DRIVE_CAPTION_MODEL` env var). Large models auto-detected by name (7b/8b/13b) and loaded with `BitsAndBytesConfig(load_in_4bit=True)` + `device_map="auto"`. Staging endpoint: `74cc374a-16fe-4c94-97d0-c78e9746fea5`.
