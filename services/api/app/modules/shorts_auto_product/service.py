...
class ProductScanService:
    ...
    async def get_scan_order_status(
        ...
        *,
        ...
        # Batch-load the underlying ShortsRenderJob rows so the wizard
        # can distinguish "scan finished, render in flight" from
        # "scan finished, render done" AND surface the migration-059
        # per-short summary on the ResultCard without a second
        # round-trip. Lazy import keeps the loose-coupling rule
        # (shorts_auto_product never module-level imports shorts_render).
        render_info_by_id: dict[
            UUID, tuple[str, str | None, datetime | None]
        ] = {}
        render_job_ids = [
        ...
        if render_job_ids:
            ...
            stmt = _select(
                ShortsRenderJob.id,
                ShortsRenderJob.status,
                ShortsRenderJob.summary,
                ShortsRenderJob.summary_generated_at,
            ).where(ShortsRenderJob.id.in_(render_job_ids))
            rows = (await self.session.execute(stmt)).all()
            render_info_by_id = {
                row.id: (row.status, row.summary, row.summary_generated_at)
                for row in rows
            }

        # Batch-resolve per-child product labels from migration 051's
        # ProductCatalogEntry. Mirrors runner.py:1205 — prefer
        # ``user_label`` over ``llm_label`` so the wizard's ResultCard
        # matches the gallery's display rule. Children with NULL
        # catalog_entry_id (whole-catalog fan-out) and parents stay
        # empty-list.
        catalog_label_by_id: dict[UUID, str] = {}
        catalog_entry_ids = [
            c.catalog_entry_id
            for c in children
        ...
        if catalog_entry_ids:
            ...
            ).where(ProductCatalogEntry.id.in_(catalog_entry_ids))
            cat_rows = (await self.session.execute(stmt_cat)).all()
            catalog_label_by_id = {
                row.id: (row.user_label or row.llm_label)
                for row in cat_rows
            }

        children_responses: list[JobStatusResponse] = []
        for c in children:
            render_info = (
                render_info_by_id.get(c.render_job_id)
                if c.render_job_id else None
            )
            child_render_status = render_info[0] if render_info else None
            child_render_summary = render_info[1] if render_info else None
            child_render_summary_at = render_info[2] if render_info else None

            child_labels: list[str] = []
            if c.catalog_entry_id is not None:
                resolved = catalog_label_by_id.get(c.catalog_entry_id)
                if resolved:
                    child_labels = [resolved]

            children_responses.append(
                _job_to_status_response(
                    c,
                    render_status=child_render_status,
                    render_summary=child_render_summary,
                    render_summary_generated_at=child_render_summary_at,
                    product_labels=child_labels,
                )
            )
        complete_count = sum(
        ...
        criteria = CriteriaSummary(
            length_seconds=getattr(parent, "length_seconds", None),
            requested_count=getattr(parent, "requested_count", None),
            time_range_start_ms=getattr(parent, "time_range_start_ms", None),
            time_range_end_ms=getattr(parent, "time_range_end_ms", None),
            product_distribution=getattr(parent, "product_distribution", None),
            intent=getattr(parent, "intent", None),
        )
...
def _job_to_status_response(
    ...
    *,
    ...
    render_summary_generated_at: datetime | None = None,
    ...
    ``render_summary`` / ``render_summary_generated_at`` mirror migration
    059's columns on ``ShortsRenderJob``; ``product_labels`` is the
    catalog label(s) resolved from migration 051's
    ``ProductCatalogEntry``. All three default empty/None so legacy
    callers (single-job endpoint, phase-4 foundation tests) keep their
    ...
    return JobStatusResponse(
        ...
        render_summary_generated_at=render_summary_generated_at,
