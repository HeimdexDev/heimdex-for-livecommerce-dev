...
class JobStatusResponse(BaseModel):
    ...
    # migration 059 — ``ShortsRenderJob.summary`` projected onto the
    # render_child for the wizard's ResultCard. ``None`` until the
    # post-render summary service writes it (or NULL forever on jobs
    # the user never ran summary on). Always ``None`` for scan_order
    # parents and enumeration jobs (no underlying render).
    render_summary: str | None = None
    render_summary_generated_at: datetime | None = None

    # migration 051 — display labels resolved from each child's
    # ``catalog_entry_id`` via ProductCatalogEntry (``user_label`` else
    # ``llm_label``, matching the gallery's display rule). Empty list
    # when no catalog entry is attached (scan_order parents,
    # whole-catalog children with NULL catalog_entry_id, enumeration
    # jobs). Future multi-product-per-child renders may carry >1 label.
...
class CriteriaSummary(BaseModel):
    """Wizard-input snapshot for the result page's option badge (Q12).

    Mirrors the relevant ``ProductScanJob`` parent columns. All fields
    optional so legacy parents created before wizard fields existed
    ...
    product_distribution: str | None = None  # 'single' | 'multi'
...
class ScanOrderStatusResponse(BaseModel):
    ...
    # Wizard-input snapshot for step4 result page's option badge (Q12).
    # Optional so legacy parents without wizard fields still validate.
