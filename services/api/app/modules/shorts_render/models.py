...
class ShortsRenderJob(Base, UUIDMixin, TimestampMixin):
    ...
    # Per-short Korean summary (migration 059). Generated by
    # ``summary_service.py`` and persisted here so the saved-shorts
    # list / preview cards can display it without an OpenAI call per
    # card. ``summary_prompt_version`` lets a prompt bump invalidate
    # cached summaries: the service regenerates when it differs from
    # the current ``summary_prompt.PROMPT_VERSION``.
