...
class TestSceneSearchClient:
    ...
    async def test_search_lexical_long_query_includes_ocr_clause(self, mock_scene_client):
        ...
        speaker_clauses = [c for c in query["should"] if "match" in c and "speaker_transcript" in c["match"]]
        assert len(title_clauses) == 1
        assert len(ocr_clauses) == 1
        assert len(caption_clauses) == 1
        assert len(speaker_clauses) == 1
        assert ocr_clauses[0]["match"]["ocr_text_norm"]["boost"] == 0.6
        assert caption_clauses[0]["match"]["scene_caption"]["boost"] == 1.0
        assert speaker_clauses[0]["match"]["speaker_transcript"]["boost"] == 0.9
