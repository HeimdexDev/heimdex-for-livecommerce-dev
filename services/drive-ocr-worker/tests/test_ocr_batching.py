"""Tests for OCR worker enrichment batching and YouTube job status skipping."""
...
class TestSafeUpdateJobStatus:
    ...
    def test_skips_youtube_video_ids(self):
        ...
        _safe_update_job_status(
            api_client, "yt_abc123", "file-uuid", job_type="ocr", status="done"
        )
    ...
    def test_calls_for_gdrive_video_ids(self):
        ...
        api_client.update_job_status.assert_called_once_with(
            "file-uuid", job_type="ocr", status="done", lease_token="tok"
        )
    ...
    def test_calls_for_regular_video_ids(self):
        ...
        api_client = MagicMock()
        _safe_update_job_status(
            api_client, "some_other_id", "file-uuid", job_type="ocr", status="failed"
        )
        api_client.update_job_status.assert_called_once_with(
            "file-uuid", job_type="ocr", status="failed"
        )
...
class TestPostEnrichToApi:
    ...
    def test_batches_at_200_scenes(self):
        ...
        # Build 450 scenes with OCR fields
        scenes = [
            {
                "scene_id": f"scene_{i}",
    ...
    def test_filters_scenes_without_ocr_text(self):
        ...
        # Mix of scenes with and without OCR
        scenes = [
            {"scene_id": "scene_1", "ocr_text_raw": "text 1", "ocr_char_count": 6},
            {"scene_id": "scene_2"},  # No OCR
            {"scene_id": "scene_3", "ocr_text_raw": "text 3", "ocr_char_count": 6},
            {"scene_id": "scene_4"},  # No OCR
        ]
    ...
    def test_empty_scenes_no_api_call(self):
        """Scenes without OCR text should return early without API calls."""
        mock_requests = MagicMock()
    ...
    def test_ocr_char_count_default(self):
        """ocr_char_count should default to length of ocr_text_raw if not provided."""
        mock_requests = MagicMock()
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"updated_count": 1}
        ...
        scenes = [
            {
                "scene_id": "scene_1",
                "ocr_text_raw": "hello world",
                # ocr_char_count not provided
            }
        ]
        ...
        call_args = mock_requests.post.call_args
        payload = call_args.kwargs["json"]
        assert payload["scenes"][0]["ocr_char_count"] == 11  # len("hello world")
